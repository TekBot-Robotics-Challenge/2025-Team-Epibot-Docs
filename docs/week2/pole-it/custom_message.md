...
However, robotics applications often require exchanging more complex or application-specific data. In such cases, **custom messages** allow us to define exactly the data structure our system needs, improving clarity, maintainability, and reducing ambiguity in communication.
## Best Practice: Creat a Package for Custom Messages
A recommended practice in ROS 2 development is to create a **dedicated package** if you want to creat  a custom messages. This approach offers several advantages:

- **Reusability:** Other packages in the project (or even in other projects) can depend on the message definitions without circular dependencies.
...
**In summary:**  
While standard messages cover many use cases, custom messages are essential for complex or unique data exchange. Organizing them in a separate package is a best practice that enhances modularity, reusability, and maintainability.


### 1. Creating the Package

To create a new ROS 2 package specifically for  a custom message definitions, we use the following command:
```bash
...
Below the explanation of every file inside the package
...
we use `float32` a floating-point value representing the different measurement.

---
 
The `DataCollect.msg` file is crucial for defining custom data structures used in ROS2 node communication. It standardizes how information is shared.

# package.xml

## Role

The `package.xml` file is a manifest file used in ROS 2 packages. It provides essential metadata about the package, such as its name, version, description, maintainers, licenses, and dependencies. This file describe how to build, install, and use the package.
...
[https://faun.pub/mastering-ros2-building-robust-communication-with-custom-messages-services-and-actions-9a49b4cd8b16]
